...
if (!canEditDonation) {
  ...
  document
    ...
    .addEventListener("submit", (e) => {
      e.preventDefault();
      const receipt = financialLocked ? donation.receiptNumber : document.getElementById("receipt").value.trim(),
        amount = financialLocked ? Number(donation.amount) : Number(document.getElementById("amount").value),
      ...
      const newStatus = financialLocked ? donation.status : document.getElementById("status").value;
      Object.assign(donation, {
        sourceType: document.getElementById("source").value,
        donorName: document.getElementById("name").value.trim(),
        pradeshikamId: financialLocked ? donation.pradeshikamId : Number(document.getElementById("pradeshikam").value),
        amount,
        receiptNumber: receipt,
        paymentMode: financialLocked ? donation.paymentMode : document.getElementById("mode").value,
        transactionId: financialLocked ? (donation.transactionId || "") : fcmsGetUpiTransactionId("mode"),
        chequeNumber: financialLocked ? (donation.chequeNumber || "") : fcmsGetChequeNumber("mode"),
        status: newStatus,
        date: new Date(
      ...
      addActivity(db, {
        ...
        details:
          wasHold && newStatus !== "hold"
            ...
            : `Donation details edited by ${actorLabel()}. ${financialLocked ? "Payment amount, receipt, mode and status were locked." : ""}`,
        oldValue: old,
