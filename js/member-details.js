...
if (!member) {
  ...
  <div class="row g-3"><div class="col-lg-4"><div class="panel h-100"><div class="panel-title mb-3">Member Information</div>${info("Phone", formatPhone(member.phone || "", member.countryCode || "+91") || "-")}${info("Marital Status", member.maritalStatus || "-")}${info("Receipt Number", member.receiptNumber || "-")}${info("House Number", member.houseNumber || "-")}${info("Pradeshikam", pr?.name || "-")}${info("Required Amount", money(member.requiredAmount))}<div class="mt-3 pt-3 border-top">${fcmsAuditIdentityHTML(member)}</div></div></div>
