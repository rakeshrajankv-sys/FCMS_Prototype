...
if(!member){location.href="members.html"}
  ...
  <div class="row g-3 mb-3"><div class="col-12"><div class="panel"><div class="d-flex justify-content-between align-items-center mb-3"><div><div class="panel-title">Household Cluster · ${escapeHTML(member.houseNumber||"-")}</div><div class="small text-muted">${household.length} member${household.length===1?"":"s"} in this house</div></div><a class="btn btn-sm btn-outline-primary" href="members.html?house=${encodeURIComponent(member.houseNumber||"")}">View House</a></div><div class="row g-2">${household.map(h=>{const hs=memberStats(h,db);return `<div class="col-md-6 col-xl-4"><div class="border rounded-3 p-2 d-flex justify-content-between align-items-center"><div><a class="text-decoration-none fw-semibold" href="member-details.html?id=${encodeURIComponent(h.id)}">${escapeHTML(h.name)}</a><div class="small text-muted">${h.gender}, ${h.age}</div></div><div class="text-end"><div class="small">${money(hs.paid)} / ${money(h.requiredAmount)}</div>${badge(hs.status)}</div></div></div>`}).join("")}</div></div></div></div>
