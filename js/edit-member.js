...
<div class="col-md-4"><label class="form-label">Marital Status *</label><select id="marital" class="form-select" required>${["Single", "Married", "Widower"].map((x) => `<option ${member.maritalStatus === x ? "selected" : ""}>${x}</option>`).join("")}</select></div>
<div class="col-md-4"><label class="form-label">Phone Number *</label><div class="input-group"><select id="country" class="form-select" style="max-width:92px"><option ${member.countryCode === "+91" ? "selected" : ""}>+91</option><option ${member.countryCode === "+971" ? "selected" : ""}>+971</option></select><input id="phone" class="form-control" inputmode="numeric" type="tel" maxlength="10" required value="${escapeHTML(normalizePhone(member.phone || ""))}"></div></div>
