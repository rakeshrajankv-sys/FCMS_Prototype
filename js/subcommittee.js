...
if (ctx) {
  ...
  const allocated = subCommitteeAllocationTotal(committee.id, db),
    ...
    collectionRemaining = subCommitteeCollectionRemaining(committee.id, db),
    expenseBalance = subCommitteeExpenseBalance(committee.id, db);
  document.getElementById("page-content").innerHTML =
    `${pageTitle(escapeHTML(committee.name), "", `<a href="subcommittee-collections.html?committee=${encodeURIComponent(committee.id)}&add=1" class="btn btn-primary"><i class="bi bi-plus-circle me-2"></i>Add Collection</a>`)}<div class="row g-3 mb-4">${statCard("bi-wallet2", "Amount Received from Main Office", money(allocated))}${statCard("bi-cash-stack", "Collected by Committee", money(collected))}${statCard("bi-bank", "Submitted to Main Committee", money(submitted))}${statCard("bi-receipt", "Expenses", money(expenses))}${statCard("bi-wallet2", "Expense Budget Remaining", money(expenseBalance))}${statCard("bi-cash-stack", "Collection Remaining", money(collectionRemaining))}</div><div class="row g-3 mb-4"><div class="col-lg-8"><div class="panel h-100"><div class="panel-title mb-3">Collection Overview</div><div class="row g-3"><div class="col-6"><div class="summary-metric"><span>Total Collected</span><b>${money(collected)}</b></div></div><div class="col-6"><div class="summary-metric"><span>Collection Remaining</span><b>${money(collectionRemaining)}</b></div></div></div></div></div><div class="col-lg-4"><div class="panel h-100"><div class="panel-title mb-3">Quick Actions</div><div class="d-grid gap-2"><a class="quick-action" href="subcommittee-collections.html?committee=${encodeURIComponent(committee.id)}&add=1"><span class="quick-icon"><i class="bi bi-cash-stack"></i></span><span><b class="d-block small">Add Collection</b></span></a><a class="quick-action" href="submissions.html?committee=${encodeURIComponent(committee.id)}"><span class="quick-icon"><i class="bi bi-bank"></i></span><span><b class="d-block small">Submit to Main Committee</b></span></a><a class="quick-action" href="subcommittee-expense.html?committee=${encodeURIComponent(committee.id)}"><span class="quick-icon"><i class="bi bi-receipt"></i></span><span><b class="d-block small">Add Expense</b></span></a></div></div></div></div><div class="panel"><div class="d-flex justify-content-between align-items-center mb-3"><div class="panel-title">Recent Collections</div><a href="subcommittee-collections.html?committee=${encodeURIComponent(committee.id)}&add=1" class="small">View all</a></div>${renderRecent()}</div>`;
