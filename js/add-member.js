...
<div class="col-md-4"><label class="form-label">Payment Mode / പേയ്മെന്റ് രീതി *</label><select id="mode" class="form-select" required><option value="Cash">Cash</option><option value="UPI">UPI</option><option value="Bank">Bank</option><option value="Cheque">Cheque</option></select></div>
...
["input", "change"].forEach((eventName) => householdForm.addEventListener(eventName, (event) => {
  const field = event.target.closest?.("input,select,textarea");
  if (field?.value !== "") field.classList.remove("is-invalid");
}));
...
async function saveHousehold(hold) {
  ...
  if (hold && total > 0) {
    ...
    const promisedPayment = {
      ...
      chequeNumber: mode === "Cheque" ? fcmsGetChequeNumber("mode") : "",
      status: "hold",
      remarks: remarks || "",
