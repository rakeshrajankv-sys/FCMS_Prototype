...
function prName(id) {
  return db.pradeshikams.find((p) => Number(p.id) === Number(id))?.name || "-";
}
function render() {
  ...
  const arr = (db.activities || []).filter(
    (a) =>
      (!pr || Number(a.pradeshikamId) === Number(pr)) &&
      (!act || a.action === act) &&
      (!actor || a.actorRole === actor) &&
      (!q ||
        [a.action, a.actor, a.summary, a.details, prName(a.pradeshikamId)]
          .join(" ")
          .toLowerCase()
          .includes(q)),
  );
  document.getElementById("activityList").innerHTML = !arr.length
...
["search", "pr", "action", "actor"].forEach((id) =>
  document
    .getElementById(id)
...
document.getElementById("exportActivity").addEventListener("click", () => {
  const data = (db.activities || []).map((a) => ({
    DateTime: new Date(a.timestamp).toLocaleString("en-IN"),
    Pradeshikam: prName(a.pradeshikamId),
    Action: a.action,
    Summary: a.summary,
    Details: a.details,
    PerformedBy: a.actor,
