...
function renderControls(){
  ...
  if(scope==="subcommittee") controlsEl.innerHTML=`<div class="row g-2 align-items-end"><div class="col-md-4"><label class="form-label">Sub Committee / ഉപസമിതി</label><select id="sc" class="form-select"><option value="">All Sub Committees</option>${db.subCommittees.map(c=>`<option value="${c.id}">${escapeHTML(c.name)}</option>`).join("")}<option value="other">Other</option></select></div><div class="col-md-3"><label class="form-label">Pradeshikam / പ്രദേശികം</label><select id="pr" class="form-select"><option value="">All Pradeshikams</option>${db.pradeshikams.map(p=>`<option value="${p.id}">${escapeHTML(p.name)}</option>`).join("")}</select></div><div class="col-md-3"><label class="form-label">Action / പ്രവർത്തനം</label><select id="action" class="form-select"><option value="">All actions</option>${allActions.map(a=>`<option>${escapeHTML(a)}</option>`).join("")}</select></div><div class="col-md-2"><label class="form-label">User / ഉപയോക്താവ്</label><select id="actor" class="form-select"><option value="">All users</option><option value="admin">Main Committee</option><option value="pradeshikam">Pradeshikam</option><option value="subcommittee">Sub Committees</option></select></div></div><div class="row g-2 mt-1"><div class="col-12"><input id="search" class="form-control" placeholder="Search Sub Committee, Pradeshikam, action or details"></div></div>`;
  else controlsEl.innerHTML=`<div class="row g-2 align-items-end"><div class="col-md-4"><label class="form-label">Pradeshikam / പ്രദേശികം</label><select id="pr" class="form-select"><option value="">All Pradeshikams</option>${db.pradeshikams.map(p=>`<option value="${p.id}">${escapeHTML(p.name)}</option>`).join("")}</select></div><div class="col-md-3"><label class="form-label">Action / പ്രവർത്തനം</label><select id="action" class="form-select"><option value="">All actions</option>${allActions.filter(a=>!a.toLowerCase().includes("sub committee")).map(a=>`<option>${escapeHTML(a)}</option>`).join("")}</select></div><div class="col-md-3"><label class="form-label">User / ഉപയോക്താവ്</label><select id="actor" class="form-select"><option value="">All users</option><option value="admin">Main Committee</option><option value="pradeshikam">Pradeshikam</option></select></div><div class="col-md-2"><input id="search" class="form-control" placeholder="Search"></div></div>`;
