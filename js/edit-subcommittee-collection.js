...
<div class="col-md-4"><label class="form-label">Amount / തുക *</label><input id="amount" type="number" min="1" class="form-control" required value="${Number(item.amount)}"></div>
<div class="col-md-4"><label class="form-label">Receipt Number / രസീത് നമ്പർ *</label><input id="receipt" class="form-control" required value="${escapeHTML(item.receiptNumber || "")}"></div>
<div class="col-md-4"><label class="form-label">Payment Mode / പേയ്മെന്റ് രീതി *</label><select id="mode" class="form-select" required>${["Cash", "UPI", "Bank", "Cheque"].map((x) => `<option ${item.paymentMode === x ? "selected" : ""}>${x}</option>`).join("")}</select></div>
