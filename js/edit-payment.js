...
if(s.role!=="admin"){location.href="payments.html"}
const id=new URLSearchParams(location.search).get("id"),payment=db.payments.find(p=>p.id===id),member=payment&&db.members.find(m=>m.id===payment.memberId);
if(!payment||!member){location.href="payments.html"}
else{
...
document.getElementById("editPaymentForm").addEventListener("submit",e=>{
 e.preventDefault();
 const receipt=document.getElementById("receipt").value.trim(),amount=Number(document.getElementById("amount").value),err=document.getElementById("formError");
 const dup=db.payments.find(p=>p.id!==payment.id&&p.receiptNumber===receipt);
 if(dup){err.textContent="That receipt number is already in use.";err.classList.remove("d-none");return}
 if(amount<0){err.textContent="Enter a valid amount.";err.classList.remove("d-none");return}
 const old=paymentSnapshot(payment);
 const otherPaid=db.payments.filter(p=>p.memberId===member.id&&p.id!==payment.id).reduce((a,p)=>a+Number(p.amount),0);
 if(member.requiredAmount>0 && otherPaid+amount>member.requiredAmount){err.textContent=`This change would exceed the member's required amount of ${money(member.requiredAmount)}.`;err.classList.remove("d-none");return}
 payment.receiptNumber=receipt;payment.amount=amount;payment.paymentMode=document.getElementById("mode").value;payment.remarks=document.getElementById("remarks").value.trim();
 addActivity(db,{action:"Payment Edited",entityType:"payment",entityId:payment.id,memberId:member.id,pradeshikamId:member.pradeshikamId,summary:`Receipt ${receipt} edited`,details:"Payment edited by Main Committee.",oldValue:old,newValue:paymentSnapshot(payment)});
