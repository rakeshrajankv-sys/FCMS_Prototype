...
<div class="panel"><div class="row g-2 mb-3"><div class="col-md-7"><input id="search" class="form-control" placeholder="Search receipt, member or phone"></div><div class="col-md-3">${s.role==="admin"?`<select id="prFilter" class="form-select"><option value="">All Pradeshikams</option>${db.pradeshikams.map(p=>`<option value="${p.id}">${escapeHTML(p.name)}</option>`).join("")}</select>`:""}</div><div class="col-md-3"><select id="mode" class="form-select"><option value="">All payment modes</option><option>Cash</option><option>UPI</option><option>Bank</option><option>Check</option></select></div><div class="col-md-2"><button id="exportBtn" class="btn btn-outline-primary w-100"><i class="bi bi-download me-1"></i>CSV</button></div></div><div id="paymentTable"></div></div>`;
function render(){
 const q=document.getElementById("search").value.toLowerCase(),mode=document.getElementById("mode").value,prFilter=document.getElementById("prFilter")?.value||"";
 const arr=rows.filter(p=>{const m=db.members.find(x=>x.id===p.memberId);return(!q||[p.receiptNumber,m?.name,m?.phone].join(" ").toLowerCase().includes(q))&&(!mode||p.paymentMode===mode)&&(!prFilter||Number(m?.pradeshikamId)===Number(prFilter))});
...
document.getElementById("mode").addEventListener("change",render);document.getElementById("prFilter")?.addEventListener("change",render);
