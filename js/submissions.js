...
async function deleteSubmission(id){
  ...
  if(target.type==="subcommittee")db.subCommitteeSubmissions=db.subCommitteeSubmissions.filter(x=>x.id!==id);else db.submissions=db.submissions.filter(x=>x.id!==id);fcmsClearPageDraft(); saveDB(db);toast("Submission deleted.","success");render();
}
